...
export class DeliveriesService {
  ...
  async findAll(search?: string, pendingOnly = false): Promise<Delivery[]> {
    ...
    if (search?.trim()) {
      conditions.push('(m.nome LIKE ? OR m.unidade LIKE ? OR e.codigo_rastreio LIKE ?)');
      const term = `%${search.trim()}%`;
      values.push(term, term, term);
    }
    if (pendingOnly) conditions.push("e.status = 'AGUARDANDO_RETIRADA'");
    const where = conditions.length ? `WHERE ${conditions.join(' AND ')}` : '';
    const [rows] = await this.database.query<Delivery[]>(
  ...
  async create(residentId: number, description: string, trackingCode: string | undefined, userId: number): Promise<Delivery> {
    if (!residentId || !description?.trim()) throw new BadRequestException('Morador e descrição são obrigatórios.');
    const [residents] = await this.database.query<RowDataPacket[]>('SELECT id FROM moradores WHERE id = ?', [residentId]);
    if (!residents.length) throw new NotFoundException('Morador não encontrado.');
    ...
    const deliveries = await this.findById(result.insertId);
    return deliveries;
  }
  ...
  async pickup(id: number, userId: number): Promise<Delivery> {
    const [result] = await this.database.execute<ResultSetHeader>(
      `UPDATE encomendas SET status = 'RETIRADA', data_retirada = CURRENT_TIMESTAMP,
        retirada_por = ? WHERE id = ? AND status = 'AGUARDANDO_RETIRADA'`,
      [userId, id],
    );
